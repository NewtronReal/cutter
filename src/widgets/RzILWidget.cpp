      ...
      mDisasTextEdit(new RzILTextEdit(this))
{
    ...
    leftPanel = new RzILLeftPanel(this);
    ...
    connect(mDisasScrollArea, &RzILScrollArea::wheelEventTriggered, this,
            &RzILWidget::showTransientScrollBar);
    ...
    connect(Core(), &CutterCore::instructionChanged, this, &RzILWidget::instructionChanged);
    connect(Core(), &CutterCore::breakpointsChanged, this, &RzILWidget::refreshIfInRange);
    connect(Core(), SIGNAL(refreshCodeViews()), this, SLOT(refreshDisasm()));

    connect(Config(), &Configuration::fontsUpdated, this, &RzILWidget::fontsUpdatedSlot);
    connect(Config(), &Configuration::colorsUpdated, this, &RzILWidget::colorsUpdatedSlot);
...
void RzILWidget::setPreviewMode(bool previewMode)
{
...
QWidget *RzILWidget::getTextWidget()
{
    return mDisasTextEdit;
}

QString RzILWidget::getWidgetType()
{
    return "RzIL";
}

QFontMetricsF RzILWidget::getFontMetrics()
{
    return QFontMetricsF(mDisasTextEdit->font());
}

QList<DisassemblyLine> RzILWidget::getLines()
{
    return lines;
}

void RzILWidget::showTransientScrollBar()
{
    mDisasScrollArea->verticalScrollBar()->showTransientScrollBar();
}

void RzILWidget::refreshIfInRange(RVA offset)
{
...
void RzILWidget::instructionChanged(RVA offset)
{
    leftPanel->clearArrowFrom(offset);
    refreshDisasm();
}

void RzILWidget::refreshDisasm(RVA offset)
{
    ...
    // Retrieve disassembly lines
    {
        ...
        lines = Core()->rzILLines(topOffset, maxLines);
    }
...
void RzILWidget::scrollInstructions(int count, bool clampToScrollBarRange)
{
    if (count == 0) {
        return;
    }
...
bool RzILWidget::updateMaxLines()
{
...
void RzILWidget::highlightCurrentLine()
{
...
void RzILWidget::highlightPCLine()
{
...
void RzILWidget::showDisasContextMenu(const QPoint &pt)
{
    mCtxMenu->exec(mDisasTextEdit->mapToGlobal(pt));
}

RVA RzILWidget::readCurrentDisassemblyOffset()
{
    QTextCursor tc = mDisasTextEdit->textCursor();
    return DH::readDisassemblyOffset(tc);
}

void RzILWidget::updateCursorPosition()
{
...
void RzILWidget::connectCursorPositionChanged(bool disconnect)
{
    if (disconnect) {
        QObject::disconnect(mDisasTextEdit, &QPlainTextEdit::cursorPositionChanged, this,
                            &RzILWidget::cursorPositionChanged);
    } else {
...
void RzILWidget::cursorPositionChanged()
{
...
void RzILWidget::moveCursorRelative(bool up, bool page)
{
    if (page) {
        RVA offset;
        if (!up) {
...
void RzILWidget::jumpToOffsetUnderCursor(const QTextCursor &cursor)
{
    RVA offset = DH::readDisassemblyOffset(cursor);
    seekable->seekToReference(offset);
}

bool RzILWidget::eventFilter(QObject *obj, QEvent *event)
{
...
void RzILWidget::keyPressEvent(QKeyEvent *event)
{
...
void RzILWidget::contextMenuEvent(QContextMenuEvent *event)
{
...
QString RzILWidget::getWindowTitle() const
{
    return tr("RzIL");
}

void RzILWidget::on_seekChanged(RVA offset, CutterCore::SeekHistoryType type)
{
...
void RzILWidget::fontsUpdatedSlot()
{
    setupFonts();
...
void RzILWidget::colorsUpdatedSlot()
{
    setupColors();
    refreshDisasm();
}

void RzILWidget::setupFonts()
{
    mDisasTextEdit->setFont(Config()->getFont());
}

void RzILWidget::setupColors()
{
...
RzILScrollArea::RzILScrollArea(QWidget *parent) : QAbstractScrollArea(parent)
{
...
AddressRangeScrollBar *RzILScrollArea::verticalScrollBar()
{
    return vScrollBar;
}

bool RzILScrollArea::viewportEvent(QEvent *event)
{
...
void RzILScrollArea::wheelEvent(QWheelEvent *event)
{
...
qreal RzILTextEdit::textOffset() const
{
    return (blockBoundingGeometry(document()->begin()).topLeft() + contentOffset()).y();
}

bool RzILTextEdit::viewportEvent(QEvent *event)
{
    switch (event->type()) {
...
void RzILTextEdit::scrollContentsBy(int dx, int dy)
{
    if (!lockScroll) {
...
void RzILTextEdit::keyPressEvent(QKeyEvent *event)
{
    Q_UNUSED(event)
    // QPlainTextEdit::keyPressEvent(event);
}

void RzILTextEdit::mousePressEvent(QMouseEvent *event)
{
...
RzILLeftPanel::RzILLeftPanel(RzILWidget *disas)
{
    this->disas = disas;
    arrows.reserve((arrowsSize * 3) / 2);
}

void RzILLeftPanel::wheelEvent(QWheelEvent *event)
{
...
void RzILLeftPanel::paintEvent(QPaintEvent *event)
{
    Q_UNUSED(event)
    ...
    auto tEdit = qobject_cast<RzILTextEdit *>(disas->getTextWidget());
...
void RzILLeftPanel::clearArrowFrom(RVA offset)
{
