...
void HexDiff::mouseMoveEvent(QMouseEvent *event)
{
    ...
    const bool cursorOnArea = ctxA.asciiArea.contains(pos)||
                            ctxB.asciiArea.contains(pos)||
                            ctxA.itemArea.contains(pos)||
                            ctxB.itemArea.contains(pos);

    auto mouseAddr = mousePosToAddrA(pos).address;

    QString infoText;
    if (!updatingSelection && (cursorOnArea)) {
        QString metaData = getFileFromPos(pos)==DiffFile::A ? getFlagsAndComment(mouseAddr,ctxA):getFlagsAndComment(getAddressB(mouseAddr),ctxB);//has to be redefined with ctx
...
void HexDiff::fillMarks(QPainter &painter, bool ascii, DiffFileContext &ctx)
{
    const uint64_t addr = ctx.file==DiffFile::A ? startAddress:getStartAddressB();

    uint64_t endAddress = addr + visibleLines * itemColumns * itemGroupSize * itemByteLen;
    endAddress = std::min(endAddress, ctx.data->maxIndex());
    ...
    for (const auto &mark : marks) {
        ...
        if (to < addr || from > endAddress) {
            continue;
        }

        const auto shapes = rangePolygons(from, to, ascii, ctx);
...
uint64_t HexDiff::ctxAddr(uint64_t addrA, ctx){
    ...
    for (int line = 0; line < visibleLines; ++line) {
        ...
        for (int j = 0; j < itemColumns; ++j) {
            for (int k = 0; k < itemGroupSize && itemAddr <= ctx.data->maxIndex();
                ...
                if (selection.contains(itemAddr) && cursorArea>1) {
...
{
    const bool fileB = ctx.file==DiffFile::B;
    if (selection.isEmpty()) {
        return;
    }
    const uint64_t start = fileB ? getAddressB(selection.start()): selection.start();
    const uint64_t end = fileB ? getAddressB(selection.end()): selection.end();
    const auto parts = rangePolygons(start, end, ascii, ctx);
...
BasicDiffCursor HexDiff::mousePosToAddrA(const QPoint &point, bool middle) const
{
    return (ctxA.asciiArea.contains(point)||ctxB.asciiArea.contains(point)) ?
            asciiPosToAddrA(point, middle) : screenPosToAddrA(point, middle);
}
