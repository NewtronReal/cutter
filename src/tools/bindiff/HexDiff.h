...
enum DiffArea : ut8{
    ...
    ItemA,
    ItemB
};//used modulo and comparisons for getting areas for cursorArea and area in mouse press event both need to be rewritten with a nullarea or no area pointer
...
private:
    ...
    void fillMarks(QPainter &painter, bool ascii, DiffFileContext &ctx);
    void updateMetrics();
    ...
    bool moveCursor(int offset, bool select = false,
                    OverflowMove overflowMove = OverflowMove::Clamp);//The entire movecursor has to cursor shall be moved in same direction in both do with BasicDiffCursor and HexDiffSelection
    ...
    inline uint64_t lastVisibleAddr(DiffFile file) const { return ((file == DiffFile::A ? startAddress:getStartAddressB()) - 1) + bytesPerScreen(); }
